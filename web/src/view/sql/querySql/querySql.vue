  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            <template #default="scope">
            <el-button type="primary" link icon="view" class="table-button" @click="catQuerySqlFunc(scope.row)">查看</el-button>
            <el-button type="primary" link icon="edit" class="table-button" @click="updateQuerySqlFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="dialogFormTitle">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        <el-form-item label="名称:"  prop="name" >
          <el-input v-model="formData.name" :clearable="true"  placeholder="请输入" :readonly="dialogFormReadonly"/>
        </el-form-item>
        <el-form-item label="分类:"  prop="cate" >
...
const getTableData = async() => {
  const table = await getQuerySqlList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteQuerySqlByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateQuerySqlFunc = async(row) => {
    const res = await findQuerySql({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 查看行
const catQuerySqlFunc = async(row) => {
  const res = await findQuerySql({ ID: row.ID })
  type.value = 'cat'
  if (res.code === 0) {
    formData.value = res.data.requerySql
...
// 删除行
const deleteQuerySqlFunc = async (row) => {
    const res = await deleteQuerySql({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
