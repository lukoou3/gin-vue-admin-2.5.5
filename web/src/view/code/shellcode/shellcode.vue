...
const getTableData = async() => {
  const table = await getShellcodeList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteShellcodeByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateShellcodeFunc = async(row) => {
...
const deleteShellcodeFunc = async (row) => {
    const res = await deleteShellcode({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createShellcode(formData.value)
                  break
                case 'update':
                  res = await updateShellcode(formData.value)
                  break
                default:
                  res = await createShellcode(formData.value)
                  break
